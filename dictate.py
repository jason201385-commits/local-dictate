...
# 強標記詞庫（2026-07-29 依 grok 的 380 條台灣口語調查擴充）。
# 放寬的風險很低：標記命中只是「進入判斷」，沒有 Case 1/2 的結構把握不會動任何字。
# ⚠️ regex 交替是先到先贏，一定要長的在前——用清單排序建構，不要手寫字串。
_STRONG_LIST = [
    "啊不對不對", "不對不對", "啊不對", "阿不對", "欸不對", "誒不對", "喔不對", "唉不對",
    "不對啦", "不對喔", "不對耶",
    "不是不是不是", "不是不是", "啊不是啦", "欸不是啦", "阿不是啦", "阿不是", "啊不是",
    "剛才講錯了", "剛剛說錯了", "剛剛講錯了", "前面講錯了",
    "我說錯了", "我講錯了", "我講錯", "說錯了", "講錯了", "錯了錯了", "啊錯了",
    "我口誤了", "我搞錯了", "啊我搞錯了", "我記錯了",
    "我更正一下", "更正一下", "更正", "抱歉說錯了", "不好意思講錯了",
    "啊等等不對", "等等不對", "等一下不對",
]
_CORR_STRONG = "|".join(re.escape(x) for x in sorted(_STRONG_LIST, key=len, reverse=True))
# 弱標記（也是一般用語）：前後都要邊界才算——grok 負面表 160 條裡的
# 不對稱/不對勁/對不對/不是嗎/我是說真的… 全靠這條邊界規則擋掉
_WEAK_LIST = ["不對", "不是", "我是說"]
_CORR_WEAK = "|".join(re.escape(x) for x in sorted(_WEAK_LIST, key=len, reverse=True))
...
_CORR_ANCHORS = ["處理", "聯絡", "打給", "傳給", "改到", "約在", "就是",
                 ...
                 # ⚠️ 單字動詞會撞複合詞（「點」撞「點餐」、「看」撞「好看」——實測踩過），
                 # 只收在名詞裡極少見的：
                 "吃", "喝", "訂", "搭", "寄"]
...
def _apply_one_correction(text):
    ...
    for L in range(min(len(a), 15), 1, -1):
        ...
        for an in _CORR_ANCHORS:
            q = a.rfind(an, 0, cut)
            if q >= 0:
                if an == "是" and q > 0 and a[q - 1] == "不":
                    continue              # 「不是」是否定/殘留標記，不是動詞錨點
