...
按一下講話，字直接出現在游標所在的地方。沒有字數上限，不用訂閱。

**聲音永遠只在你的電腦裡轉成文字，不會上傳。** 唯一會送出東西的是可選的「整理」功能，而且送的是**已經轉好的文字**、不是聲音，面板上點一下就能關掉。
...
**「聲音會不會被上傳？」**
不會。錄音和辨識都在你電腦上跑，拔掉網路線也能用。唯一會送出去的是可選的「整理」功能，送的是**已經轉好的文字**、不是聲音，面板點一下就能關。

**「防毒軟體跳警告怎麼辦？」**
有可能會跳，**先告訴你原因**：這個程式要註冊全域熱鍵、讀寫剪貼簿、還會幫你按 `Ctrl+V`——這三件事跟鍵盤側錄程式的行為特徵重疊，所以有些防毒會示警。程式碼全部在這個 repo 裡可以自己看。不放心的話，先只在自己的筆電試，不要裝在公司電腦。

**「免費的，那你賺什麼？」**
不賺。這是我自己要用才做的，做完順手開源。MIT 授權，拿去改、拿去用、拿去賣都可以。
...
市面上的 AI 聽寫工具大多是雲端的（音訊要上傳、有字數上限、要月費）。而幾個「本來應該可以用」的選項，實際踩下去是這樣（2026-07 實測，之後可能會變；本專案與下列產品皆無任何關聯，各商標屬其權利人所有）：
...
本專案 MIT。拿去改、拿去用、拿去賣都可以。

安裝檔打包的第三方元件各依其原授權散布，完整清單與原文見 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)（隨安裝檔一併安裝）。其中 pynput 與 FFmpeg 是 LGPL——你有權用自己的版本替換它們，做法見 [LGPL-COMPLIANCE.md](LGPL-COMPLIANCE.md)。
